...
from backend.app.application.planning_service import apply_plan_adjustment, reject_plan_adjustment, request_replan
...
def reject_plan_adjustment_endpoint(
    adjustment_id: str,
    payload: RejectPlanAdjustmentRequest,
    principal: Principal = Depends(get_current_principal),
    session: Session = Depends(get_session),
) -> dict:
    try:
        return reject_plan_adjustment(
            session,
            adjustment_id=adjustment_id,
            user_id=principal.user_id,
            goal_id=payload.goal_id,
