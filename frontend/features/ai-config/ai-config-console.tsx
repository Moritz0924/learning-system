    ...
    <>
      ...
      <DetailPane title={selected ? selected.name : t("config.newModel")} subtitle={t("config.openaiEndpoint")}>
        <form onSubmit={(event) => void save(event)} className="space-y-5">
          ...
          <Footer busy={busy} result={result} onDelete={selected ? () => { if (window.confirm(t("config.deleteConfirm", { name: selected.name }))) void removeModel(selected.id).then(() => { setSelectedId("new"); return refresh(); }).catch(() => setResult(t("config.deleteFailed"))); } : undefined} />
        </form>
      </DetailPane>
    </>
  );
  ...
  return <>
    ...
    <DetailPane title={selected ? selected.name : t("config.newSkill")} subtitle={t("config.skillHelp")}>
      <form onSubmit={(event) => void save(event)} className="space-y-5">
        ...
        <Footer busy={busy} result={result} onDelete={selected ? () => { if (window.confirm(t("config.deleteConfirm", { name: selected.name }))) void removeSkill(selected.id).then(() => { setSelectedId("new"); return refresh(); }).catch(() => setResult(t("config.deleteFailed"))); } : undefined} />
      </form>
    </DetailPane>
  </>;
}
  ...
  return <>
    ...
    <DetailPane title={selected ? selected.name : t("config.newMcp")} subtitle={t("config.mcpHelp")}>
      <form onSubmit={(event) => void save(event)} className="space-y-5">
        ...
        <Footer busy={busy} result={result} onDelete={selected ? () => { if (window.confirm(t("config.deleteConfirm", { name: selected.name }))) void removeMcpServer(selected.id).then(() => { setSelectedId("new"); return refresh(); }).catch(() => setResult(t("config.deleteFailed"))); } : undefined} />
      </form>
    </DetailPane>
  </>;
}
